...
then
	echo ""
	echo "TCP Tweaker network settings already added in the system!"
	echo ""
...
sysctl -p /etc/sysctl.conf > /dev/null
		echo ""
		echo "TCP Tweaker network settings removed successfully."
		echo ""
	exit
	else
		echo ""
	...
	echo ""
	read -p "Continue with the installation? [s/n]: " -e -i n resposta
	if [[ "$resposta" = 's' ]]; then
	echo ""
...
sysctl -p /etc/sysctl.conf
		echo ""
		echo "TCP Tweaker network settings added successfully."
		echo ""
	else
		echo ""
